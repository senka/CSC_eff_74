...
#   arg2 "lct_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+arg2 and "seg_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+arg2 are the directory name in the TagandProbe result file;
...
def GetEff(f_in,path="lct_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo",effcat="fit_eff"):
    print " trying..."
    try:
        print "   ... tryinh"
#        print " -----> status: ",f_in.Get("Probes/"+path+"/fitresults").statusCodeHistory(2)," ",eff.statusCodeHistory(1), " ",eff.statusCodeHistory(0)
        eff=f_in.Get("Probes/"+path+"/fitresults")
        print " -----> status: ",eff.statusCodeHistory(2)," ",eff.statusCodeHistory(1), " ",eff.statusCodeHistory(0)
        return [eff.statusCodeHistory(2),eff.statusCodeHistory(1),eff.statusCodeHistory(0)]
    except:
        print "\033[97mOther problems, skip",f_in.GetName(),"\033[0m"
        return [0.]*3

def GetBinnedEffPlot(f_in,path="lct_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo",effcat="fit_eff",st_=0,name_=plotname):
        ...
        while (ikey!=list_.After(list_.Last())):
            ...
            if binnumber:
                ...
                if ibin<nbins and ibin>=0:
                    ...
                    if result_:
                        value_=result_.floatParsFinal().find("statusCodeHistory(0)")
...
if "Stations" in Group:
    ...
    for idx in range(1,n_stations+1):
        ...
        if "pt" in Group or "eta" in Group or "phi" in Group:
            LCTEff=GetBinnedEffPlot(f_in, "lct_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+Postfix,categoryname,stations[idx][3])
            SEGEff=GetBinnedEffPlot(f_in, "seg_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+Postfix,categoryname,stations[idx][3])
            file_out.cd()
            if LCTEff:
                LCTEff.Write()
            if SEGEff:
            ...
            Effs.append( GetEff(f_in, "lct_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+Postfix,categoryname)+GetEff(f_in,"seg_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+Postfix,categoryname) )
            f_in.Close()
    if not ("pt" in Group or "eta" in Group or "phi" in Group):
        Effs=array(Effs).transpose()*1.
...
#        LCTEff.SetMinimum(90)
    ...
    SEGEff=TH2F("SEGEff","fit status MINOS (-HESSE) (+MIGRAD)",36,1,37,20,-9,9)
  ...
  #split tree to chamber
    for idx in range(n_chambers):
        ...
        f_in=TFile(filename_,"READ");
        if Postfix=="_MCTruth":
            Effs=GetEff(f_in, "lct_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+Postfix,"cnt_eff")+GetEff(f_in,"seg_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+Postfix,"cnt_eff")
        else:
	    print "reading file: ", f_in, " reading efficiency: ", "seg_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+Postfix,"fit_eff"
            Effs=GetEff(f_in, "lct_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+Postfix,"fit_eff")+GetEff(f_in,"seg_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+Postfix,"fit_eff" )
        f_in.Close()
        iBin_y=RingToYMap[(st,rg)]
        iBin_y=11+iBin_y if ec else 10-iBin_y
        eff=Effs[0]*1.
	LCTEff.SetBinContent(ch,iBin_y,Effs[0]*1.);
        LCTEff_downErr.SetBinContent(ch,iBin_y,Effs[1]*1.);
        LCTEff_upErr.SetBinContent(ch,iBin_y,Effs[2]*1.);
        eff=Effs[3]*1.
        print "  -> setting content: ", Effs[3]*1.," ", Effs[4]*1., " ", Effs[5]*1.
	SEGEff.SetBinContent(ch,iBin_y,Effs[3]*1.);
        SEGEff_downErr.SetBinContent(ch,iBin_y,Effs[4]*1.);
        SEGEff_upErr.SetBinContent(ch,iBin_y,Effs[5]*1.);
    gStyle.SetPaintTextFormat("4.1f")
...
#    LCTEff.SetMinimum(0)
    file_out.cd()
    SegCanvas.Write()
    SegCanvas.SaveAs("SegCanvas_2D_Run2015B_NOHLTIsoTkMu_fit0.pdf")
    SegCanvas.SaveAs("SegCanvas_2D_Run2015B_NOHLTIsoTkMu_fit0.root")
    SegCanvas.SaveAs("SegCanvas_2D_Run2015B_NOHLTIsoTkMu_fit0.C")
    LCTCanvas.Write()
    LCTCanvas.SaveAs("LCTCanvas_2D_Run2015B_NOHLTIsoTkMu_fit0.pdf")
    LCTCanvas.SaveAs("LCTCanvas_2D_Run2015B_NOHLTIsoTkMu_fit0.root")
    LCTCanvas.SaveAs("LCTCanvas_2D_Run2015B_NOHLTIsoTkMu_fit0.C")
    SEGEff.Write()
    SEGEff_upErr.Write()
    ...
    if not os.path.isfile(filename_):
        ...
        f_in=TFile(filename_,"READ");
        categoryname="cnt_eff" if Postfix=="_MCTruth" else "fit_eff"
        LCTEff=GetBinnedEffPlot(f_in, "lct_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+Postfix,categoryname)
        SEGEff=GetBinnedEffPlot(f_in, "seg_effV/abseta_bin0__shiftedphi_bin0__tracks_pt_bin0__VoigtianPlusExpo"+Postfix,categoryname)
        f_in.Close()
        file_out.cd()
        if LCTEff:
           LCTEff.Write("LCTEff")
